...
import (
	...
	"github.com/detiber/cluster-api-ansible/pkg/ansible"
...
const (
	ansibleMachineControllerName = "ansible-machine-controller"
)
...
func StartMachineController(server *options.MachineControllerServer, shutdown <-chan struct{}) {
	...
	// TODO: Setup a config watch for controller-specific config
	actuator, err := ansible.NewMachineActuator()
	if err != nil {
		glog.Fatalf("Could not create Ansible machine actuator: %v", err)
	}
...
func Run(server *options.MachineControllerServer) error {
	...
	rl, err := resourcelock.New(
		...
		ansibleMachineControllerName,
		leaderElectionClient.CoreV1(),
		resourcelock.ResourceLockConfig{
			Identity:      id + "-" + ansibleMachineControllerName,
...
func createRecorder(kubeClient *kubernetes.Clientset) (record.EventRecorder, error) {
	...
	return eventBroadcaster.NewRecorder(eventsScheme, corev1.EventSource{Component: ansibleMachineControllerName}), nil
}
